...
docker run -it -v $(pwd)/.empirica:/data/.empirica nubaeon/empirica:1.5.6-alpine /bin/sh
```
...
```bash
# CLI help
docker run --rm nubaeon/empirica:1.5.6-alpine empirica --help

# Interactive shell
docker run -it --rm nubaeon/empirica:1.5.6-alpine /bin/sh

# With persistent data volume
docker run -it \
  -v $(pwd)/.empirica:/data/.empirica \
  nubaeon/empirica:1.5.6-alpine \
  empirica session-create --ai-id docker-agent

# Check version
docker run --rm nubaeon/empirica:1.5.6-alpine empirica --version
```
...
|-----|------|------|----------|
| `1.5.6-alpine` | Alpine 3.23 | ~85MB | Production, security-hardened |
| `1.5.6` | Debian Bookworm | ~150MB | Development, glibc compatibility |

---

## Key Features in 1.5.6
