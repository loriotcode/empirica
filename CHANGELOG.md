...
- **Stale `active_work.json` fallback** - `get_active_project_path()` now rejects `active_work.json` older than 4 hours instead of silently resolving to wrong project after reboot
