...
### Fixed
- **Non-tmux instance isolation** - Instance IDs for X11 (`x11_N`) and macOS Terminal (`term_N`) now use underscores matching the file naming convention. Previously used colons (`x11:N`) causing filename mismatch — Sentinel couldn't find transaction files, failing open silently. See known issue 11.20
- **project-switch in non-tmux** - `_update_active_work()` now uses canonical `get_instance_id()` instead of tmux-only logic. Writes `instance_projects` for all environment types (X11, macOS Terminal, TTY)
...
### Changed
- **Instance isolation docs** - Updated architecture docs to reflect full non-tmux support. Instance projects are now keyed by any `instance_id` type, not just tmux panes
