...
- **Stale project resolution after reboot** - `session-init` now fires on `resume` events, updating anchor files for the new terminal. Previously fell through to stale `active_work.json` from prior session
...
### Changed
- **Instance isolation docs** - Simplified ARCHITECTURE.md and README.md to reflect InstanceResolver, staleness checks, and resume handling
