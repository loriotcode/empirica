...
> **Startup exception (v1.8.0):** On `startup` events, `session-init.py` validates the resolved project against CWD's git root. If CWD is a valid Empirica project and differs from the stale instance file, CWD wins. On `resume`/`compact`/`clear`, instance files remain authoritative.
...
4. **No CWD fallback (mid-session)** — Fail explicitly rather than risk wrong project context. Exception: `startup` events prefer CWD over stale instance files (v1.8.0)
