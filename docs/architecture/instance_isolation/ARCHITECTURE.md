...
**Separation rationale (v1.8.6):** Hooks doing read-modify-write on the transaction file could race with POSTFLIGHT's status=closed write. Splitting into two files gives single-writer semantics per file.
...
Project Path (get_active_project_path):
  ...
  ⚠️  STARTUP EXCEPTION (v1.8.6): session-init prefers CWD over stale instance files on 'startup' events
