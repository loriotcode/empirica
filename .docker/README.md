...
```bash
# Get help
docker run --rm nubaeon/empirica:1.6.2 empirica --help

# Check version
docker run --rm nubaeon/empirica:1.6.2 empirica --version

# Create a session
docker run --rm nubaeon/empirica:1.6.2 empirica session-create --ai-id docker-agent --output json
```
...
```bash
docker run -v /path/to/your/data:/data nubaeon/empirica:1.6.2
```
...
```bash
docker inspect nubaeon/empirica:1.6.2 | jq '.[0].Config.Healthcheck'
```
...
docker run -p 8080:8080 nubaeon/empirica:1.6.2 \
